...
const DYNAMIC_RENDER = 'setDynamicRenderDeps';
const WRAP_WITH_SLOT = 'wrapWithSlot';
 ...
 */
const addOrUpdateEventListener = (node: Element, event: string, listener: (...args: any[]) => void) => {
  ...
  if (listener !== undefined) {
    const handleEvent = (e: Event) => {
      ...
      if (detail && detail._isDetailArgs) {
        listener.apply(node, detail);
      } else {
        listener.call(node, detail);
      }
    };
...
const setProperty = <E extends Element>(node: E, name: string, value: unknown, old?: unknown, events?: EventNames) => {
  ...
  if (event !== undefined) {
    if (value !== old) {
      addOrUpdateEventListener(node, event, value as (...args: any[]) => void);
    }
    return;
  }
...
export const createComponent = <I extends HTMLElement, E extends EventNames = {}, S extends SlotNames = {}>({
  ...
  const ReactComponent = React.forwardRef<I, Props>((props, ref) => {
    ...
    const [dynamicChildren, setDynamicRenderDeps] = React.useState<unknown[]>(
      elementClass?.__internal_renderDynamicSlots?.(props, wrapWithSlot) || [],
    );
    ...
    if (!NODE_MODE) {
      ...
      React.useLayoutEffect(() => {
        ...
        setProperty(elementRef.current, WRAP_WITH_SLOT, wrapWithSlot);
        setProperty(elementRef.current, DYNAMIC_RENDER, setDynamicRenderDeps);
    ...
    const hostElement = React.createElement(tagName, {
      ...reactProps,
      children: [reactProps.children, ...dynamicChildren, ...Object.values(childrenProps)],
    ...
    if (props.appendToBody && typeof document !== 'undefined') {
      return createPortal(hostElement, document.body);
    }
