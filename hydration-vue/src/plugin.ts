...
export const vueOutputTarget = (outputTarget: OutputTargetVue): OutputTargetCustom => ({
  ...
  async generator(config, compilerCtx, buildCtx) {
    ...
    await vueProxyOutput(config, compilerCtx, outputTarget, buildCtx.components, buildCtx.config);
    const { plugins = [] } = outputTarget;
    for (let i = 0; i < plugins.length; i++) {
      const plugin = plugins[i];
      await plugin(config, compilerCtx, outputTarget, buildCtx.components);
    }
...
export function normalizeOutputTarget(config: Config, outputTarget: any) {
  const results: OutputTargetVue = {
    ...
    tagNameSuffix: outputTarget.tagNameSuffix || '',
  };

  if (!config.rootDir) {
    throw new Error('rootDir is not set and it should be set by stencil itself');
  }
  if (!outputTarget.proxiesFile) {
  ...
  if (outputTarget.directivesProxyFile && !path.isAbsolute(outputTarget.directivesProxyFile)) {
    results.proxiesFile = normalizePath(path.join(config.rootDir as string, outputTarget.proxiesFile));
  }
