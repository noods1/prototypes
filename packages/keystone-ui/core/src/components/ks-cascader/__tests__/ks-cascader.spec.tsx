...
describe('KsCascader', () => {
  ...
  it('should render with default props', async () => {
    const page = await newSpecPage({
      components: [KsCascader, KsCascaderColumns],
      template: () => <ks-cascader />,
    });

    // 模拟 columnsEl
    page.rootInstance.columnsEl = {
      initColumnList: jest.fn(),
      updateScrollIntoView: jest.fn(),
    };
  ...
  it('should set queryString when searchValue prop is provided', async () => {
    const page = await newSpecPage({
      components: [KsCascader, KsInput, KsCascaderSearchlist, KsCascaderColumns],
      template: () => <ks-cascader searchable={true} searchValue="test" />,
    });
  ...
  it('should emit ksSearchChange event when search input changes', async () => {
    const page = await newSpecPage({
      components: [KsCascader, KsInput, KsCascaderSearchlist, KsCascaderColumns],
      template: () => <ks-cascader searchable={true} />,
    });
  ...
  it('should not update queryString when searchValue is controlled', async () => {
    const page = await newSpecPage({
      components: [KsCascader, KsInput, KsCascaderSearchlist, KsCascaderColumns],
      template: () => <ks-cascader searchable={true} searchValue="controlled" />,
    });
